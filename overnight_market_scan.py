#!/usr/bin/env python3
"""
V50.8.2.2.1a AI Committee Scanner - Exclusions + Insider Intelligence
...
# =========================
# V50.8.2.2.1a SCANNER STATE VERSION LOCK
# =========================
SCANNER_VERSION = "V50.8.2"
SCANNER_VERSION_LABEL = "V50.8.2 Data Reliability Patch"

def scanner_version_value() -> str:
    return SCANNER_VERSION

def apply_scanner_version(state):
    """
    Force every persisted scanner state to use the active scanner version.
    This prevents old V41/V42/V47 fallback state from leaking into the dashboard.
    """
    if isinstance(state, dict):
        state["version"] = SCANNER_VERSION
...
def write_json(path: Path, data: Any) -> None:
    if path == STATE_FILE and isinstance(data, dict):
        data = apply_scanner_version(data)
...
def build_price_history_intelligence(df: pd.DataFrame, ind: Dict[str, Any]) -> Dict[str, Any]:
    """
    V50.8.2.2.1a.3 Official Economic Calendar Fallback.
...
def v42_build_committee_safe(symbol: str, row: Dict[str, Any], meta: Dict[str, Any], ind: Dict[str, Any], hist: pd.DataFrame) -> Dict[str, Any]:
    """
    V50.8.2.2.1a.7 safety wrapper around V42 committee creation.
...
# =========================
# V50.8.2.2.1a FAST TIERED SCAN
# =========================
# V50.8.2.2.1a.1 hard cron controls
...
# V50.8.2.2.1a.1 safety timeout patch: prevents one slow endpoint from dragging cron.
...
def v421_should_run_full_research(symbol: str, row: Dict[str, Any]) -> bool:
    """
    V50.8.2.2.1a.1 hard cap: scheduled cron full committee is only for the very best names.
...
def v421_apply_tiered_committee(symbol: str, row: Dict[str, Any], meta: Dict[str, Any], ind: Dict[str, Any], hist: pd.DataFrame) -> Dict[str, Any]:
    """
    V50.8.2.2.1a tiered scheduled scan:
    ...
    for i in range(0, len(universe), BATCH_SIZE):
        ...
        for symbol in batch:
            ...
            if meta is None:
                ...
                # V50.8.2.2.1a.1: skip expensive deep APIs during pre-rank pass.
...
# V50.8.2.2.1a marker: Advisor-Style Decision Engine with GitHub Actions persistence compatibility.


# V50.8.2.2.1a marker: Institutional Research Completion Engine compatibility.


# V50.8.2.2.1a marker: Research Quality Correction Patch.


# V50.8.2.2.1a marker: Client-Friendly Advisor Language Patch.


# V50.8.2.2.1a marker: Execution Readiness Engine.


# V50.8.2.2.1a marker: Clean Trust Engine.


# V50.8.2.2.1a marker: Trust UX + Analyst Targets.
