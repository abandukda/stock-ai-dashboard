...
VERSION="ATLAS_PERFORMANCE_SNAPSHOT_V2"
        ...
        for line in path.read_text().splitlines():
            try:
                ...
                if prior.get("timestamp"):
                    normalized_prior=_timestamp(prior.get("timestamp"),field="DECISION").isoformat()
                    points[(prior.get("ticker"),normalized_prior)]=prior["snapshot_id"]
            except Exception: continue
    fresh=[]
    for raw in snapshots:
        s=dict(raw);s["timestamp"]=_timestamp(s.get("timestamp"),field="DECISION").isoformat();point=(s.get("ticker"),s.get("timestamp"));prior_id=points.get(point)
...
def mature_snapshot(snapshot: Mapping[str,Any], bars: Sequence[Mapping[str,Any]], benchmark_bars: Sequence[Mapping[str,Any]]=()) -> list[dict[str,Any]]:
    start=_num(snapshot.get("price")); decision_at=_timestamp(snapshot.get("timestamp"),field="DECISION")
    def observed_at(item: Mapping[str,Any], *, field: str) -> datetime:
        return _timestamp(item.get("timestamp") or item.get("date") or item.get("datetime"),field=field)
    dated_bars=[(observed_at(x,field="OUTCOME_BAR"),x) for x in bars]
    future=[x for observed,x in sorted(dated_bars,key=lambda pair:pair[0]) if observed>decision_at and _num(x.get("close")) is not None]
    prices=[_num(x.get("close")) for x in future]
    dated_benchmark=[(observed_at(x,field="BENCHMARK_BAR"),x) for x in benchmark_bars]
    benchmark_start=next(((_num(item.get("close")),observed) for observed,item in reversed(sorted(dated_benchmark,key=lambda pair:pair[0])) if observed<=decision_at and _num(item.get("close")) is not None),(None,None))
    if not start:return []
    out=[]
    for h in HORIZONS:
        ...
        end_at=observed_at(bar_window[-1],field="OUTCOME_BAR")
        benchmark_end=next(((_num(item.get("close")),observed) for observed,item in reversed(sorted(dated_benchmark,key=lambda pair:pair[0])) if decision_at<observed<=end_at and _num(item.get("close")) is not None),(None,None))
        bret=(benchmark_end[0]/benchmark_start[0]-1) if benchmark_start[0] and benchmark_end[0] is not None else None
