...
def validate_valuation(row:Mapping[str,Any])->dict[str,Any]:
    ...
    for model in models:
        ...
        if model.get("status")=="PUBLISHED" and model.get("methodology_id")=="VAL_EV_EBITDA_V1":
            ...
            ev_checks.append({"enterprise_value":enterprise,"net_debt":net_debt,"equity_value":enterprise-net_debt if enterprise is not None and net_debt is not None else None,"diluted_shares":shares,"reconstructed_per_share":reconstructed,"published_per_share":model.get("value"),"discrepancy_pct":round(gap*100,4) if gap is not None else None,"status":"PASS" if gap is not None and gap<=.01 else "FAIL"})
