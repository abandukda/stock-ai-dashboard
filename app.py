...
# VERSION
# =====================
APP_VERSION = "V13 FINAL - PERSONAL WATCHLIST + FULL DASHBOARD"
...
def load_personal_watchlist():
    raw = cookie_manager.get(cookie="personal_watchlist_v13")
...
def save_personal_watchlist(symbols):
    ...
    cookie_manager.set(
        cookie="personal_watchlist_v13",
        val=json.dumps(clean),
        expires_at=datetime.now() + timedelta(days=365),
        key="save_personal_watchlist_v13"
    )
...
if st.sidebar.button("Logout"):
    ...
    cookie_manager.delete(cookie="ai_dashboard_auth", key="delete_auth_cookie_v13")
    st.rerun()
...
with tab6:
    ...
    if not journal_df.empty:
        closed = journal_df[journal_df["Result"] != "Open"]
...
### 📬 Alerts
Alerts trigger only when a stock becomes actionable.
