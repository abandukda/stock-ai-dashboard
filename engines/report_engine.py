...
Every page should eventually call render_stock_report().
"""
...
def _fmt_score(value: Any) -> str:
    return f"{_safe_num(value, 0):.0f}/100"


def _fmt_pct(value: Any) -> str:
    return f"{_safe_num(value, 0):.1f}%"
...
def build_why_we_like_it(row: dict[str, Any]) -> list[str]:
    ...
    analyst = _safe_text(
        row.get("Analyst Support")
        or row.get("Analyst View"),
        "",
    )
...
def build_key_risks(row: dict[str, Any]) -> list[str]:
    ...
    thesis_risk = _safe_text(
        row.get("Primary Risk")
        or row.get("Risk")
        or row.get("What Could Go Wrong"),
        "",
    )
...
def render_investment_scorecard(row: dict[str, Any]) -> None:
    ...
    with c4:
        metric_card(
            "Modeled Upside",
            _fmt_pct(row.get("Target Upside %") or row.get("Upside")),
        )

    with c5:
        metric_card(
            "Risk / Reward",
            _safe_text(row.get("Risk/Reward") or row.get("Risk Reward"), "N/A"),
        )

    with c6:
        metric_card(
            "Political Signal",
            _safe_text(row.get("Political Signal"), "Coming soon"),
        )
...
def render_entry_plan(row: dict[str, Any]) -> None:
    ...
    with c2:
        metric_card(
            "Stop / Risk Control",
            _safe_text(row.get("Stop") or row.get("Stop Loss"), "Use risk controls"),
        )
    ...
    with c3:
        metric_card(
            ...
            _safe_text(
                row.get("Target")
                or row.get("AI / Analyst Target")
                or _fmt_money(row.get("AI Fair Value")),
                "Review target",
            ),
        )
...
def render_stock_report(row: dict[str, Any], source: str = "Latest AI Market Scan") -> None:
    ...
    confidence = _fmt_score(
        row.get("Confidence")
        or row.get("Research Confidence")
        or row.get("AI Confidence")
    )

    clean_recommendation = clean_recommendation_text(recommendation)
    ...
    executive_summary = (
        ...
        or (
            f"Atlas currently rates {ticker} as {clean_recommendation} and classifies it as "
            f"{classification}. The latest scan shows an opportunity score of "
            f"{_fmt_score(row.get('Opportunity') or row.get('Final Conviction') or row.get('Score'))}, "
            f"quality score of {_fmt_score(row.get('Quality') or row.get('Financial Health') or row.get('Finance Agent Score'))}, "
            f"and modeled upside of {_fmt_pct(row.get('Target Upside %') or row.get('Upside'))}. "
